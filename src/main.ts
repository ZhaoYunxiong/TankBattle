...
if (import.meta.env.DEV) Object.defineProperty(window, '__tankBattle', { get: () => ({
  ...
  rendering: { craters: renderer.scene.meshes.filter(m => m.name === 'persistent-crater').length, fallen: renderer.scene.meshes.filter(m => m.name === 'fallen-log').length, meshes: renderer.scene.meshes.length, active: renderer.scene.getActiveMeshes().length, tracks: renderer.scene.meshes.filter(m => m.name === 'track-print').length },
  vehicle: (() => {

...
get('app').innerHTML = '<main id="menu" class="menu-screen">' +
  ...
  '<div class="scene-label"><span>EXPLORE / DEFEND</span><b id="mapTitle">薄雾山谷</b><p>循路探营，穿林伏击。</p></div><footer class="menu-footer"><button id="helpButton" class="help-link">操作手册 ↗</button><span>为每一位童年的坦克手</span><a href="https://github.com/ZhaoYunxiong/TankBattle" target="_blank" rel="noreferrer">GITHUB ↗</a></footer></main>' +
...
function renderLobby() {
  ...
  const signature = players.map(t => t.id + t.name + t.ready + t.connected).join('|') + rooms.role + state.mode + state.difficulty + state.mapSize;
...
function drawRadar() {
  ...
  for (let z = -mapFor(state.mapSize).arena.z; z < mapFor(state.mapSize).arena.z; z += EXPLORE_STEP) for (let x = -mapFor(state.mapSize).arena.x; x < mapFor(state.mapSize).arena.x; x += EXPLORE_STEP) {
    ...
    const p = position(x, z); ctx.fillStyle = '#344c47'; ctx.fillRect(p[0], p[1], EXPLORE_STEP * scale + 0.2, EXPLORE_STEP * scale + 0.2);
  }
...
function updateHud() {
  ...
  if (state.enemyBaseDiscovered && !discoveredBefore) toast('发现敌军营地！位置已同步到小队地图。', 2500);
  discoveredBefore = state.enemyBaseDiscovered;
  previousBaseHp = state.baseHp;
...
if (import.meta.env.DEV) Object.defineProperty(window, '__tankBattle', { get: () => ({
  ...
  rendering: { meshes: renderer.scene.meshes.length, active: renderer.scene.getActiveMeshes().length, tracks: renderer.scene.meshes.filter(m => m.name === 'track-print').length },
  vehicle: (() => {

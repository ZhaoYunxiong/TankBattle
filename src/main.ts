...
import { ENGINEER, VEHICLES, vehicleStats, type TankKind } from './game/vehicles';
import { BOOST, chargePower } from './game/abilities';
import { ambushReady, IMPACTS } from './game/tactics';
import type { ImpactKind } from './game/types';
...
get('app').innerHTML = '<main id="menu" class="menu-screen">' +
  ...
  '<dialog id="lobbyDialog"><div class="dialog-content"><div class="dialog-header"><h2>山谷小队</h2><button id="leaveLobby" class="icon-button" aria-label="离开房间">' + icon('close') + '</button></div><div class="room-code"><div><small>邀请朋友，一起守卫</small><strong id="roomCode"></strong></div><img id="roomQr" alt="扫码加入房间"></div><button id="copyRoom" class="text-button">复制邀请链接 ↗</button><div id="lobbyMode" class="lobby-mode"><b></b><span></span><small>模式、难度和地图由房主在开房前选择</small></div><button id="lobbyVehicle" class="vehicle-entry" type="button" aria-label="更换本局坦克"><span><small>本局车型 · 点击更换</small><b id="lobbyVehicleName">标准突击型</b></span><i>↗</i></button><div id="players" class="players"></div><button id="startRoom" class="primary">开始守卫</button><button id="readyButton" class="primary" hidden>我准备好了</button><p id="lobbyStatus">正在等待队友。最多 4 人，房主也可以独自出发。</p><p>房主切到后台会暂停战场；离开房间会结束本次联机。开房使用公共配对服务，战场通过设备直连同步。</p></div></dialog>' +
  ...
  '<dialog id="helpDialog"><div class="dialog-content"><div class="dialog-header"><h2>坦克手册</h2><button class="icon-button" data-close="helpDialog" aria-label="关闭手册">' + icon('close') + '</button></div><p>经典模式：守住己方营地，摧毁敌军营地获胜；防守模式：击退五波来袭敌军。两种模式均可单人或合作，坦克被击毁后可使用两辆备用坦克，己方营地被毁则战役结束。</p><table class="help-table"><tr><td>选择车型</td><td>出征前打开车库选择标准、轻型、重型或工程坦克；可拖动查看三维模型。新车型用工厂金币永久解锁，数值升级适用于全部车型。联机各人独立选车，换车后重新准备，开战后固定本局车型</td></tr><tr><td>工程维修</td><td>脱战 8 秒后，7 米内每 10 秒自修 8 点、维修友军或友塔 12 点。维修双方须脱战且无遮挡，多车不叠加；维修他人获得贡献分，每局最多 300 分</td></tr><tr><td>电脑驾驶</td><td>WASD / 方向键按镜头方向移动，车身自动转向</td></tr><tr><td>电脑瞄准</td><td>单击战场锁定鼠标；拖动/鼠标移动瞄准，左键或空格开火</td></tr><tr><td>自由镜头</td><td>滚轮缩放，Alt 只观察，C 归位</td></tr><tr><td>手机操作</td><td>左摇杆推向哪里就往哪里走，右侧拖动瞄准；按住开火按钮也能拖动</td></tr><tr><td>加速模式</td><td>点击双箭头按钮或按 B 切换。速度提高 60%，标准型满耐力约持续 8 秒，轻型更长、重型较短；停止加速 1 秒后恢复，耗尽后自动关闭</td></tr><tr><td>蓄力模式</td><td>点击闪电按钮或按 Q 切换。按住发射键蓄力，松开发射；蓄满时间和伤害依车型而定，可在车库查看；蓄力保留连发道具次数。暂停、失焦和取消触控会取消蓄力</td></tr><tr><td>防御塔与占领</td><td>经典模式小 / 中 / 大地图双方各 1 / 2 / 3 座塔，另有 1 / 2 / 3 座中立塔；防守模式保留我方塔和中立塔。驶入圈内停留 6 秒即可占领；敌我争夺时暂停，占领后自动防守，也可被敌军夺回。毁塔永久失效，营地可直接攻击</td></tr><tr><td>补给点</td><td>占领后为圈内受伤友军恢复 35% 血量，共享 30 秒冷却。满血不消耗维修，争夺期间暂停补给；小地图 1 个、中地图 2 个、大地图 3 个</td></tr><tr><td>受损坦克</td><td>低于 60% 开始冒烟、散布增加，受损减速最多 15%；维修后恢复</td></tr><tr><td>山谷地形</td><td>中央谷道与两侧高地由缓坡连通；上坡略慢，山坡可挡炮。瞄准目标时炮管自动适配高低差</td></tr><tr><td>方向装甲与弱点</td><td>双方坦克按车身方向判断：重型前方 120° 减伤 20%，所有车型后方 120° 受到额外 25% 伤害，侧面正常。伏击与弱点增伤合计最多 35%，满血坦克仍不会被一炮击毁。蓝白火花是正面装甲，橙色火花和短暂提示表示弱点命中。</td></tr><tr><td>高草伏击</td><td>深入高草后隐蔽，持续隐蔽 1.5 秒准备首炮：伤害 +15%、散布减少 60%。开炮暴露 5 秒，重新隐蔽并准备后才能再次强化；连发只有第一枚强化。草丛外敌军全局可见，发现敌营后小队全图开启。</td></tr><tr><td>河流与桥梁</td><td>浅河可涉水，速度为平地的 60%，车身深入浅水后减伤 20%；桥面无减伤，深河只能从桥上通过。</td></tr><tr><td>战术破坏</td><td>炸开树木和岩壁开辟捷径；倒木与弹坑整局保留且可碾过。树倒稳后，进入倒木范围减伤 25%；与浅水不叠加，双方遵守相同规则</td></tr><tr><td>战场补给</td><td>绿：回血（满血时保留）；黄：快装 60 / 90 / 120 秒；橙：24 次三连发；蓝：90 点护盾。重复拾取可补充，上限为两份。靠近箱子可查看效果，驶过即可拾取</td></tr></table><p>难度可选休闲、普通、挑战。普通单人经典模式，小 / 中 / 大地图最多同时 3 / 5 / 7 辆敌军，总计 18 / 28 / 40 辆；清掉进攻部队后有 20 / 22.5 / 25 秒反攻窗口。中、大地图每轮进攻满 60 秒后可补足在场缺额；增援耗尽后仍需摧毁敌营才能获胜。</p><p>敌军炮口闪光表示即将开火，利用山坡和掩体脱离视线。小队每击毁 3 辆敌军，会在击杀者身旁补充一个维修包；普通单人防守模式，小 / 中 / 大地图每波 4～8 / 6～10 / 8～12 辆，波间休整 15 / 17.5 / 20 秒。</p><p>合作模式没有队友伤害，也不会误伤本方围墙；经典模式可以摧毁敌军围墙。跨网络直连可能受运营商限制；同一可互访 Wi-Fi 下更适合一起游玩。</p></div></dialog>';
...
let outgoingImpact = { until: 0, impact: 'normal' as ImpactKind, ambush: false };
let receivedImpact = { until: 0, impact: 'normal' as ImpactKind };
...
const garage = new VehiclePicker(career, async kind => {
  if (screen === 'game' || networkBusy) return false;
  if (screen === 'lobby') {
    if (rooms.role === 'guest') {
      rooms.selectVehicle(kind);
      const deadline = performance.now() + 6000;
      await new Promise<void>(resolve => {
        const check = () => {
          if (state.tanks.find(t => t.id === localId)?.kind === kind || state.phase !== 'lobby' || performance.now() >= deadline) resolve();
          else window.setTimeout(check, 80);
        };
        check();
      });
      if (state.tanks.find(t => t.id === localId)?.kind !== kind) { toast('本局车型未更换，请等待连接恢复后重试。'); return false; }
    } else if (!simulation.selectVehicle(localId, kind)) return false;
  }
  const selected = await career.select(kind);
  syncVehicleUi(); return selected;
}, () => screen !== 'game' && !networkBusy);

function syncVehicleUi() {
  get('selectedVehicleName').textContent = VEHICLES[career.profile.selectedVehicle].name;
  if (screen === 'menu') simulation.selectVehicle(localId, career.profile.selectedVehicle);
  garage.render();
}
function openGarage() {
  if (screen === 'lobby' && rooms.role === 'guest') rooms.ready(false);
  garage.open(screen === 'lobby' ? state.tanks.find(t => t.id === localId)?.kind : undefined);
}
career.onChange = syncVehicleUi;
career.onVehicles = openGarage;
get('vehicleButton').onclick = openGarage;
get('lobbyVehicle').onclick = openGarage;
syncVehicleUi();
...
function enterGame() {
  ...
  damageToastAt = 0; lastHitDirection.until = 0; outgoingImpact.until = receivedImpact.until = 0;
...
function newSolo() {
  ...
  career.refresh();
  simulation.addPlayer(localId, nickname(), career.profile.upgrades, career.profile.selectedVehicle);
...
function menu() {
  ...
  career.refresh();
  simulation.addPlayer('preview', nickname(), career.profile.upgrades, career.profile.selectedVehicle);
  localId = 'preview';
  state = simulation.state;
  state.tanks[0].z = mapFor(state.mapSize).base.z - 15;
  previousLobby = '';
...
function renderLobby() {
  ...
  const signature = players.map(t => t.id + t.name + t.kind + t.ready + t.connected).join('|') + rooms.role + state.mode + state.difficulty + state.mapSize + state.baseMaxHp + state.campUpgrades.repair;
  if (signature === previousLobby) return;
  previousLobby = signature;
  get('players').innerHTML = players.map((t, i) => '<div class="player-slot"><i style="background:' + COLORS[t.color % 4] + '"></i><span>' + escape(t.name) + (t.id === localId ? ' · 你' : '') + '<em>' + VEHICLES[t.kind].name + '</em></span><small>' + (i === 0 ? '房主' : t.ready ? '已准备' : '准备中') + '</small></div>').join('') +
  ...
  get('lobbyVehicleName').textContent = VEHICLES[state.tanks.find(t => t.id === localId)?.kind ?? 'standard'].name;
...
async function connect(host: boolean) {
  ...
  simulation.addPlayer(localId, nickname(), career.profile.upgrades, career.profile.selectedVehicle);
      ...
      career.refresh();
      await rooms.join(get<HTMLInputElement>('roomInput').value, nickname(), career.profile.upgrades, career.profile.selectedVehicle);
...
rooms.onJoin = (id, name, upgrades, kind) => {
  const existing = state.tanks.find(t => t.id === id);
  if (!existing && state.phase !== 'lobby') return false;
  const result = simulation.addPlayer(id, name, upgrades, kind);
...
rooms.onVehicle = (id, kind) => { if (simulation.selectVehicle(id, kind)) { previousLobby = ''; rooms.broadcast(state); } };
...
function updateHud() {
  ...
  get('concealmentStatus').textContent = concealed(player, state) ? ambushReady(player, state) ? '隐蔽 · 首炮强化' : '隐蔽 · 伏击准备' : player.exposedUntil > state.time ? '暴露 ' + Math.ceil(player.exposedUntil - state.time) + 's' : '深入高草可隐蔽';
  ...
  const charging = player.charging && player.hp > 0, percent = Math.round(chargePower(player.charge, player.kind) * 100);
  ...
  get('weaponDescription').textContent = controls.chargeMode ? '蓄满伤害 ' + VEHICLES[player.kind].chargeDamage + ' · 松开发射' : VEHICLES[player.kind].name + ' · 按住连续射击';
  ...
  get('reloadBar').style.width = charging ? percent + '%' : Math.max(0, 1 - player.cooldown / ((controls.chargeMode ? vehicleStats(player.kind, player.upgrades).chargeReload : vehicleStats(player.kind, player.upgrades).reload) * (player.buffs.rapid > 0 ? 0.7 : 1))) * 100 + '%';
  get('pauseVehicle').textContent = VEHICLES[player.kind].name;
  get('repairStatus').hidden = player.kind !== 'engineer' || player.hp <= 0;
  get('repairStatus').textContent = player.combatUntil > state.time || player.charging ? '维修等待脱战' : player.repairCooldown > 0 ? '维修 ' + Math.ceil(player.repairCooldown) + 's' : '维修就绪 · ' + ENGINEER.range + 'm';
  ...
  for (const event of state.events) if (event.id > previousDropEvent) {
    ...
    if (event.kind === 'hit' && event.victim === localId && event.sourceX !== undefined && event.sourceZ !== undefined) lastHitDirection = { x: event.sourceX, z: event.sourceZ, until: state.time + 1.2 };
    if (event.kind === 'hit' && event.impact) {
      if (event.owner === localId && (outgoingImpact.until <= state.time || IMPACTS[event.impact].priority >= IMPACTS[outgoingImpact.impact].priority)) outgoingImpact = { until: state.time + 0.9, impact: event.impact, ambush: !!event.ambush };
      if (event.victim === localId && event.impact !== 'normal' && (receivedImpact.until <= state.time || IMPACTS[event.impact].priority >= IMPACTS[receivedImpact.impact].priority)) receivedImpact = { until: state.time + 1.1, impact: event.impact };
    }
  }
  previousDropEvent = state.events.at(-1)?.id ?? 0;
  const feedback = get('hitFeedback'), receipt = get('impactStatus');
  feedback.hidden = outgoingImpact.until <= state.time || state.paused || player.hp <= 0;
  feedback.textContent = (outgoingImpact.ambush ? '伏击 · ' : '') + IMPACTS[outgoingImpact.impact].label;
  feedback.dataset.impact = outgoingImpact.impact;
  feedback.style.color = IMPACTS[outgoingImpact.impact].color;
  receipt.hidden = receivedImpact.until <= state.time || state.paused || player.hp <= 0;
  receipt.textContent = IMPACTS[receivedImpact.impact].received;
  receipt.dataset.impact = receivedImpact.impact;
  receipt.style.color = IMPACTS[receivedImpact.impact].color;
  drawRadar();
  updateThreat();
  if ((state.phase === 'won' || state.phase === 'lost') && !resultShown) {
    ...
    get('resultRanking').innerHTML = standings(state).map(t => '<div><span>第 ' + (1 + state.tanks.filter(o => o.team === 'player' && o.score > t.score).length) + ' 名 · ' + escape(t.name) + (t.id === localId ? '（你）' : '') + ' · ' + VEHICLES[t.kind].name + '<small>击毁 ' + t.stats.kills + ' · 助攻 ' + t.stats.assists + ' · 防守 ' + t.stats.defenses + ' · 据点贡献 ' + t.stats.objectives + ' · 维修 ' + Math.round(t.stats.repairs) + ' · 团队奖励 ' + t.stats.teamBonus + '</small></span><b>' + t.score + '</b></div>').join('');
...
renderer.engine.runRenderLoop(() => {
  ...
  // 查看车库时保留战场背景，避免手机同时持续绘制两个三维场景。
  if (!get<HTMLDialogElement>('vehicleDialog').open) renderer.render(state, localId, dt, screen !== 'game');
  get('boot')?.remove();
  if (screen === 'game') {
    const player = state.tanks.find(t => t.id === localId);
    ...
    if (player && player.hp > 0) {
      ...
      const size = (24 + Math.max(0, 0.6 - player.hp / player.maxHp) * 38) * (ambushReady(player, state) ? 0.8 : 1);
...
if (import.meta.env.DEV) Object.defineProperty(window, '__tankBattle', { get: () => ({
  ...
  vehicle: (() => {
    ...
    return root && chassis ? { parts: root.getChildMeshes().map(m => m.name), position: { x: root.position.x, y: root.position.y, z: root.position.z }, pitch: chassis.rotation.x, roll: chassis.rotation.z } : null;
  })(),

...
import { boardKey, buyUpgrade, loadProfile, newProfile, parseProfile, PROFILE_KEY, rankName, resetFactory, saveProfile, settle, type MatchRecord, type Profile } from './profile';
...
export class Career {
  ...
  constructor(private notify: (message: string) => void) {
    ...
    document.body.insertAdjacentHTML('beforeend', `<dialog id="careerDialog"><div class="dialog-content">
      ...
      <section id="recordPanel"><label for="recordGroup">本机通关排行榜</label><select id="recordGroup"></select><div id="recordBoard"></div><p>按模式、难度、地图、人数及整队装备分榜。同分时用时较短者靠前。这里是本机记录，不是联网全球榜。</p><b class="minor-title">最近战役</b><div id="recordHistory"></div>
      ...
      <section id="factoryPanel" hidden><p>金币用于升级，累计荣誉不会减少。装备在下一局出发时生效，每项最多三级；初始装备也能完成普通战役。</p><div id="factoryItems" class="factory-items"></div><p>合作阵地按小队中每项最高等级生效一次，不按人数叠加；开房后可在小队面板查看。</p><button id="resetFactory" class="text-button">重置工厂 · 全额返还金币</button></section>
    </div></dialog>`);
  ...
  render() {
    ...
    select.innerHTML = groups.map(([key, r], i) => `<option value="${safe(key)}">${MODES[r.mode].name} · ${DIFFICULTIES[r.difficulty].name} · ${MAPS[r.mapSize].name} · ${r.players} 人 · 装备组 ${i + 1}</option>`).join('');
    if (groups.some(([key]) => key === selected)) select.value = selected;
    select.disabled = groups.length === 0; this.renderBoard();
    el('recordHistory').innerHTML = p.records.slice(0, 6).map(r => `<div class="history-row"><span>${r.won ? '胜利' : '再战'} · ${MAPS[r.mapSize].name}<small>${DIFFICULTIES[r.difficulty].name} · 个人第 ${r.rank} / ${r.players} 名</small></span><b>+${r.honor}<small>荣誉</small></b><b>+${r.coins}<small>金币</small></b></div>`).join('') || '<p class="empty-record">出发吧，第一场战役正在等你。</p>';
  }
}

...
export interface MapDefinition {
  ...
  rivers: (Region & { kind: 'shallow' | 'deep' })[];
  bridges: Region[];
  grass: Region[];
  roads: Point[][];
  restBonus: number;
...
const small: MapDefinition = {
  ...
  bridges: [{ x: 0, z: -4, width: 7, depth: 12 }, { x: -30, z: -4, width: 7, depth: 12 }],
...
const medium: MapDefinition = {
  ...
  bridges: [{ x: -20, z: -14, width: 8, depth: 16 }, { x: 44, z: -14, width: 8, depth: 16 }],
...
const large: MapDefinition = {
  ...
  bridges: [{ x: 14, z: 54, width: 18, depth: 8 }, { x: 14, z: -54, width: 18, depth: 8 }],
...
export function waterAt(p: Point, map: MapDefinition) {
  ...
  return map.rivers.find(r => inRegion(p, r))?.kind ?? null;
}

export function waterBlocked(p: Point, map: MapDefinition, radius: number) {
  return map.rivers.some(r => r.kind === 'deep' && inRegion(p, r, radius)) &&

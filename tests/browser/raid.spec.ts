...
test('普通单人经典模式探索、过桥、回攻与通关', async ({ page }) => {
  ...
  const drive = async (axis: 'x' | 'z', target: number) => {
    ...
    try {
      await page.waitForFunction(({ axis, target }) => {
        ...
        const arrived = s.tanks[0].hp <= 0 || (axis === 'x' ? s.tanks[0].x >= target : s.tanks[0].z <= target);
        if (arrived) window.dispatchEvent(new KeyboardEvent('keyup', { code: axis === 'x' ? 'KeyA' : 'KeyW', bubbles: true }));
        return arrived;
      ...
      while (Date.now() < deadline) {
        ...
        try {
          await page.waitForFunction(right => {
            const s = (window as any).__tankBattle.state;
            const arrived = s.phase !== 'battle' || s.tanks[0].hp <= 0 || (right ? s.tanks[0].x >= 26.7 : s.tanks[0].x <= 25.3);
            if (arrived) window.dispatchEvent(new KeyboardEvent('keyup', { code: right ? 'KeyA' : 'KeyD', bubbles: true }));
            return arrived;
          }, right, { timeout: 4000 });
        } finally { await page.keyboard.up(key); }
        right = !right;
  ...
  await expect(page.locator('#resultRanking')).toContainText('第 1 名');
  await expect(page.locator('#resultReward')).toContainText('本局荣誉 +');
  const profile = await page.evaluate(() => JSON.parse(localStorage.getItem('tb-profile-v1')!));
  expect(profile.wins).toBe(1); expect(profile.honor).toBeGreaterThan(1000); expect(profile.coins).toBeGreaterThan(0);
  expect(profile.settled).toEqual([result.matchId]);
  await page.reload(); await page.locator('#careerButton').click();
  await expect(page.locator('#recordBoard')).toContainText(String(result.tanks[0].score));
});
